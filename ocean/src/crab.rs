...
impl Crab {
    ...
    fn catch_prey(&mut self) -> Option<(Box<dyn Prey>, usize)> {
        ...
        for reef in self.reefs.iter(){
            ...
            if let Some(prey) = reef_borrow.take_prey() {
    ...
    fn release_prey(&mut self, prey: Box<dyn Prey>, reef_index: usize) {
        ...
        if reef_index < self.reefs.len(){
            let mut reef_borrow = self.reefs[reef_index].borrow_mut();
    ...
    pub fn hunt(&mut self) -> bool {
        ...
        while result == false {
            if let Some((mut prey, reef_index)) = self.catch_prey() {
                // Check if the caught prey is edible
                if prey.diet() == self.diet {
                    if !prey.try_escape(self){
                        result = true;
                        break;
                    }else{
                        escaped_prey.push(prey);
                    }
                ...
                // No more prey in any reefs, stop hunting
                break;
            }
        }
    
        // Release
        for (index, prey) in escaped_prey.into_iter().enumerate() {
            self.release_prey(prey, index);
        }
    
        // Return true if prey was caught, false otherwise
        return result;
    }
